<!-- resources/views/livewire/resguardo-area.blade.php -->

<div>
    <!-- ... -->
    <div class="p-6 bg-white border-b border-gray-200">
        <!-- Formulario para seleccionar el área y empleado -->
        <form wire:submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Selección de área -->
                <div>
                    <label for="area_id" class="block text-sm font-medium text-gray-700">Seleccionar Área</label>
                    <select wire:model="selectedArea" id="area_id" name="area_id"
                ...
                <div>
                    <label for="empleado_id" class="block text-sm font-medium text-gray-700">Seleccionar
                        Empleado</label>
                    <select wire:model="selectedEmpleado" id="empleado_id" name="empleado_id"
                        class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm">
                        <option value="">--Seleccionar--</option>
                        @foreach ($empleados as $empleado)
                            <option value="{{ $empleado->id }}">{{ $empleado->nombre }}
                                {{ $empleado->apellido_paterno }} {{ $empleado->apellido_materno }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <!-- Mostrar los equipos según el área y/o empleado seleccionado -->
        @if ($equipos)
            <h3 class="mt-6 text-lg font-medium text-gray-900">EQUIPOS EN EL ÁREA/EMPLEADO SELECCIONADO</h3>
            <div class="overflow-x-auto mt-2">
                <table
                    ...
                    <thead class="bg-gray-300 dark:bg-gray-700">
                        <tr>
                            <th scope="col"
                                class="px-6 text-center py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">
                                No.Inventario</th>
                            <th scope="col"
                                class="px-6 text-center py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">
                                Marca</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($equipos as $equipo)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                <td class="px-6 text-center py-4 whitespace-nowrap">{{ $equipo->numero_inventario }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-sm font-medium text-gray-500 text-center">NO HAY
                                    EQUIPOS REGISTRADOS EN ESTA BÚSQUEDA</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if ($selectedArea && !$selectedEmpleado)
                <form wire:submit.prevent="generateAreaPDF">
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-gray-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 mt-4">
                        <svg class="w-6 h-6 text-gray-100 dark:text-white" aria-hidden="true"
                            ...
                            <path fill-rule="evenodd"
                                ...
                                clip-rule="evenodd" />
                        </svg>DESCARGAR RESGUARDO POR ÁREA
                    </button>
                </form>
            @endif
        @endif
