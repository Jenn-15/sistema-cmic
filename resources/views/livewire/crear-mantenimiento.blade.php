<form class="md:w-1/2 space-y-5" wire:submit.prevent='crearMantenimiento' >
    <div>
        <x-input-label for="equipo" :value="__('Equipo:')" />
        <select id="equipo" wire:model="equipo"
            class="border-gray-300 focus:border-red-400 focus:ring-red-400 rounded-md shadow-sm w-full">
            <option>-- Seleccione --</option>
            @foreach ($equipos as $equipo)
                <option value="{{ $equipo->id }}">
                    {{ $equipo->numero_inventario }}-{{ $equipo->marca->marca }}-{{ $equipo->modelo }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <textarea wire:model="descripcion" placeholder="Descripción del mantenimiento"
    ...
    <div>
        ...
        <x-text-input id="fecha"
            ...
            type="date" wire:model="fecha" :value="old('fecha')" />
        @error('fecha')
            <livewire:mostrar-alerta :message="$message" />
        @enderror
    </div>
    ...
    <div>
        ...
        <x-text-input id="costo"
            class="block mt-1 w-full class border-gray-300 focus:border-red-400 focus:ring-red-400 rounded-md shadow-sms"
            type="text" wire:model="costo" :value="old('costo')" placeholder='Costo del mantenimiento' />
        @error('costo')
            <livewire:mostrar-alerta :message="$message" />
        @enderror
    </div>
