<div>
    <div>
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div>
                <h1 class="text-center text-2xl text-gray-700 font-extrabold mb-5">MANTENIMIENTOS</h1>
                    ...
                    <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-black uppercase bg-gray-400 dark:bg-gray-700 dark:text-gray-500">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">Costo del mantenimiento</th>
                                <th scope="col" class="px-6 py-3">Último mantenimiento</th>
                        ...
                        <tbody class="bg-white">
                            @forelse ($mantenimientos as $mantenimiento)
                                <tr class="bg-gray-50 border-b dark:bg-gray-800 dark:border:gray-700">
                                    <td class="px-6 py-4">{{ $mantenimiento->equipo->numero_inventario }}</td>
                                    <td class="px-6 py-4">{{ $mantenimiento->equipo->marca->marca }}</td>
                                    <td class="px-6 py-4">{{ $mantenimiento->equipo->modelo }}</td>
                                    <td class="px-6 py-4">{{ $mantenimiento->equipo->area->area }}</td>
                                    <td class="px-6 py-4">{{ $mantenimiento->costo }}</td>
                                    <td class="px-6 py-4">
                                        {{ Carbon\Carbon::parse($mantenimiento->fecha)->toFormattedDateString() }}
                                    </td>
                                    <td class="px-6 py-4"> <!-- Ajuste aquí -->
                                <div class="flex justify-center items-center">
                                    ...
                                    <button wire:click="$dispatch('mostrarAlerta', {{ $mantenimiento->id }})" type="button"
                                        class="font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                            ...
                            @empty
                                <tr>
                                    <td  class="p-3 text-center text-sm text-gray-600">
                                        No hay nada que mostrar.</td>
                                </tr>
                            @endforelse
                            {{ $mantenimientos->links() }} <!-- Controles de paginación -->
                        </tbody>
                    </table>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            Livewire.on('mostrarAlerta', mantenimientoId => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        @this.call('eliminarMantenimiento', {
                            mantenimientoId
                        })
